...
def build_local_graph_html(
    ...
    *,
    ...
    The first node is treated as the selected/focus node. Large logical layers are wrapped
    across multiple physical columns so a repository with many related steps/methods does not
    create an extremely tall SVG that renders as a thin line when fitted to the browser.
    """
    ...
    for node in sorted_nodes:
        ...
        node_svg.append(
            f'<g class="node{selected_class}" transform="translate({x},{y})" data-details="{metadata_json}">'
...
<div class="layout">
  <div class="canvas-wrap">
    <div class="toolbar">
      ...
      <span class="graph-summary">Focused view: {node_count} nodes · {edge_count} relationships</span>
    </div>
  ...
  <aside class="details" id="details"><h2>{safe_title}</h2><p>The orange node is the selected result. This is a focused relationship view; complete immediate incoming/outgoing relationships remain available in the ARE desktop window.</p></aside>
</div>
<script>

"""Separate local window for low-confidence repository-health review candidates."""
...
class HealthReviewWindow:
    """Grouped, paged popup dedicated to Review-severity findings."""
    ...
    def __init__(
        ...
        *,
        ...
        self._groups = group_health_findings(report, severity=HealthSeverity.REVIEW)
        ...
        self.window.title("ARE — Review Candidates")
        self.window.geometry("1240x820")
        self.window.minsize(960, 640)
        self.window.protocol("WM_DELETE_WINDOW", self.close)

        self._title_var = tk.StringVar(value="Review candidates")
    ...
    def show(self) -> None:
        """Bring an existing Review window to the front."""
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            root,
            text=(
                "These are not confirmed defects. ARE could not prove a static usage relationship, "
                "so verify them before cleanup. Findings are grouped by check type."
            ),
            wraplength=1150,
        ).pack(anchor=tk.W, fill=tk.X, pady=(3, 10))

        total = sum(group.count for group in self._groups)
        ttk.Label(
            root,
            text=f"{total} Review candidate(s) across {len(self._groups)} issue type(s).",
        ...
        groups_frame = ttk.LabelFrame(pane, text="Grouped Review Issues", padding=8)
        details_frame = ttk.LabelFrame(pane, text="Review Details", padding=8)
        pane.add(groups_frame, minsize=360, stretch="always")
        pane.add(details_frame, minsize=560, stretch="always")

        ttk.Label(
            groups_frame,
            text="Select an issue type. Similar findings stay grouped instead of flooding the screen.",
            wraplength=430,
    ...
    def _render_groups(self) -> None:
        cards = tuple(
            CardSpec(
                id=group.id,
                eyebrow=f"Review • {group.count} finding(s)",
        ...
        self._group_cards.set_cards(
            ...
            empty_text="No Review candidates were found.",
        )
    ...
    def _render_findings(self) -> None:
        if self._selected_group_id is None:
            self._title_var.set("No Review issue selected.")
            ...
            self._finding_cards.set_cards(
                ...
                empty_text="No Review candidates to display.",
            )
        ...
        self._finding_cards.set_cards(
            ...
            empty_text="No resolvable nodes were available for this Review page.",
        )
    ...
    def _open_finding(self, card_id: str) -> None:
        ...
        source = f"Health Review: {group.check_name}" if group else "Health Review"

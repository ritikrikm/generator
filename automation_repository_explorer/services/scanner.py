...
IGNORED_DIRECTORIES = {".git", ".idea", ".mvn", ".settings", "__MACOSX", "__pycache__", "allure-results", "build", "classes", "coverage", "dist", "generated-test-sources", "htmlreport", "log", "maven-archiver", "maven-status", "node_modules", "out", "surefire-reports", "target", "test-output", "testreport-archive"}
...
class RepositoryScanner:
    ...
    def scan(self, repository_path: Path, progress_callback: ProgressCallback | None = None) -> tuple[RepositoryFile, ...]:
        ...
        for root, directories, file_names in os.walk(repository_path):
            directories[:] = [directory for directory in directories if directory.lower() not in IGNORED_DIRECTORIES]
            root_path = Path(root)
